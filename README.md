...
| 操作 | 控制方式 |
|------|---------|
| **向左移动** | 键盘: ← 或 A / 虚拟按钮: ← (所有设备) |
| **向右移动** | 键盘: → 或 D / 虚拟按钮: → (所有设备) |
| **射击** | 键盘: SPACE / 移动端自动射击 |
| **移动到位置** | 点击/触摸屏幕位置（飞船自动移动到目标） |
| **暂停/继续** | 键盘: ESC / 屏幕右上角暂停按钮 |
| **重启游戏** | 键盘: SPACE (游戏结束后) 或 R (暂停时) |

**💡 提示**: 虚拟按钮（←/→）在所有平台（PC、平板、手机）都可用，按下时会有视觉反馈。
...
- ✅ 玩家控制（移动/射击，支持方向键 + WASD）
- ✅ 移动端触控支持（点击移动 + 虚拟按钮 + 自动射击）
- ✅ 响应式设计（自动适配所有屏幕尺寸，支持横竖屏）
- ✅ 敌人生成和移动
- ✅ 碰撞检测和伤害系统
- ✅ 敌人被击中闪烁效果
- ✅ 玩家被击中反馈（HIT! 文字 + 闪烁动画 + 1秒无敌时间）
- ✅ 暂停/继续（ESC 暂停，R 键重启）
- ✅ 得分和生命系统
- ✅ 游戏结束和重启
- ✅ 高分记录系统（localStorage 持久化 + 破纪录动画 + 破纪录立即保存）
- ✅ 波次系统（5波循环，难度递增，通关后保留进度）
- ✅ 背景音乐（可暂停）
- ✅ 完整的 GitHub Pages 部署
...
*游戏版本: 1.5*
*最后更新: 2025-11-16*
*特性: 移动端触控支持、虚拟按钮、响应式设计、玩家被击中反馈、WASD 键支持、5波循环系统、难度递增、高分记录、背景音乐*
